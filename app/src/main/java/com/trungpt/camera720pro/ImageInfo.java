...
public class ImageInfo implements Serializable {
    ...
    private final String LOG_TAG = "ImageInfo";
	...
	private void onLoadExifData( String path ) {
		if ( null != path ) {
            ...
            try {
                mExif.readExif( path, ExifInterface.Options.OPTION_ALL );
            } catch( IOException e ) {
                e.printStackTrace();
                return;
            }
            ...
            // datetime
            string = mExif.getTagStringValue( ExifInterface.TAG_DATE_TIME );
            if( null != string ) {
                dateTime = ExifInterface.getDateTime( string, TimeZone.getDefault() ).toString();
            }

            // orientation
            integer = mExif.getTagIntValue( ExifInterface.TAG_ORIENTATION );
            if( null != integer ) {
                if( integer.intValue() != 0 ) {
                    orientation = mExif.getRotationForOrientationValue( (short) integer.intValue() );
                }
            }

            // image width/height
            int[] imagesize = mExif.getImageSize();
            if( null != imagesize && imagesize.length == 2 ) {
                width = imagesize[0];
                height = imagesize[1];
            }

            int exif_width = 0, exif_height = 0;

            integer = mExif.getTagIntValue( ExifInterface.TAG_IMAGE_WIDTH );
            if( null != integer ) {
                exif_width = integer.intValue();
            }

            integer = mExif.getTagIntValue( ExifInterface.TAG_IMAGE_LENGTH );
            if( null != integer ) {
                exif_height = integer.intValue();
            }

            if( exif_width > 0 && exif_height > 0 ) {
                Log.d( LOG_TAG, "replacing width/height with exif_width/exif_height" );
                width = exif_width;
                height = exif_height;
            }
            ...
            process = parseProcess( mExif.getJpegProcess() );
            quality = mExif.getQualityGuess();

            exifVersion = mExif.getTagStringValue( ExifInterface.TAG_EXIF_VERSION );

            StringBuilder sb = new StringBuilder();

            string = mExif.getTagStringValue( ExifInterface.TAG_MAKE );
            if( null != string ) {
                sb.append( string );
            }

            string = mExif.getTagStringValue( ExifInterface.TAG_MODEL );
            if( null != string ) {
                if( sb.length() > 0 ) {
                    sb.append( "/" );
                }
            ...
            software = mExif.getTagStringValue( ExifInterface.TAG_SOFTWARE );
            artist = mExif.getTagStringValue( ExifInterface.TAG_ARTIST );

            string = mExif.getTagStringValue( ExifInterface.TAG_COPYRIGHT );
            if( null != string ) {
                string = string.trim();
                if( string.length() > 0 ) {
                    copyright = string;
            ...
            string = mExif.getTagStringValue( ExifInterface.TAG_DATE_TIME_DIGITIZED );
            if( null != string ) {
                dateTimeDigitized = ExifInterface.getDateTime( string, TimeZone.getDefault() ).toString();
            }


            string = mExif.getTagStringValue( ExifInterface.TAG_DATE_TIME_ORIGINAL );
            if( null != string ) {
                dateTimeOriginal = ExifInterface.getDateTime( string, TimeZone.getDefault() ).toString();
            }

            integer = mExif.getTagIntValue( ExifInterface.TAG_FLASH );
            if( null != integer ) {
                flash = processFlash( integer.intValue() );
            }

            rational = mExif.getTagRationalValue( ExifInterface.TAG_FOCAL_LENGTH );
            if( null != rational ) {
                decimalFormatter.setMaximumFractionDigits( 1 );
                String value = decimalFormatter.format( rational.toDouble() ) + "mm";

                integer = mExif.getTagIntValue( ExifInterface.TAG_FOCAL_LENGTH_IN_35_MM_FILE );
                if( null != integer ) {
                    value += " (35mm equivalen: " + integer.intValue() + ")";
                }

                focalLength = value;
            }

            rational = mExif.getTagRationalValue( ExifInterface.TAG_DIGITAL_ZOOM_RATIO );
            if( null != rational ) {
                double value = rational.toDouble();
                if( value > 1.0 ) {
                    decimalFormatter.setMaximumFractionDigits( 1 );
                    digitalZoom = decimalFormatter.format( value );
                }
            }

            rational = mExif.getTagRationalValue( ExifInterface.TAG_EXPOSURE_TIME );
            if( null != rational ) {
                decimalFormatter.setMaximumFractionDigits( 6 );
                exposureTime = decimalFormatter.format( rational.toDouble() ) + "s";
            }

            rational = mExif.getTagRationalValue( ExifInterface.TAG_SHUTTER_SPEED_VALUE );
            if( null != rational ) {
                double value = rational.toDouble();
                if( value > 0 ) {
                    decimalFormatter.setMaximumFractionDigits( 1 );
                    shutterSpeed = "1/" + decimalFormatter.format( Math.pow( 2, value ) ) + "s";
                    exposureTime = null;
                }
            }
            ...
            integer = mExif.getTagIntValue( ExifInterface.TAG_ISO_SPEED_RATINGS );
            if( null != integer ) {
                sb.append( "ISO-" + integer.intValue() );
            }
            ...
            rational = mExif.getTagRationalValue( ExifInterface.TAG_BRIGHTNESS_VALUE );
            if( null != rational ) {
                brightness = String.valueOf( rational.toDouble() );
            }

            integer = mExif.getTagIntValue( ExifInterface.TAG_COLOR_SPACE );
            if( null != integer ) {
                colorSpace = processColorSpace( integer.intValue() );
            }

            rational = mExif.getTagRationalValue( ExifInterface.TAG_SUBJECT_DISTANCE );
            if( null != rational ) {
                double distance = rational.toDouble();
                if( distance > 0 ) {
            ...
            integer = mExif.getTagIntValue( ExifInterface.TAG_SUBJECT_DISTANCE_RANGE );
            if( null != integer ) {
                subjectDistanceRange = processSubjectDistanceRange( integer.intValue() );
            }

            // exposure bias
            rational = mExif.getTagRationalValue( ExifInterface.TAG_EXPOSURE_BIAS_VALUE );
            if( null != rational ) {
                if( rational.toDouble() != 0 ) {
                    exposureBias = String.valueOf( rational.toDouble() );
                }
            }

            integer = mExif.getTagIntValue( ExifInterface.TAG_WHITE_BALANCE );
            if( null != integer ) {
                whiteBalance = processWhiteBalance( integer.intValue() );
            }

            integer = mExif.getTagIntValue( ExifInterface.TAG_LIGHT_SOURCE );
            if( null != integer ) {
                lightSource = processLightSource( integer.intValue() );
            }

            integer = mExif.getTagIntValue( ExifInterface.TAG_METERING_MODE );
            if( null != integer ) {
                meteringMode = processMeteringMode( integer.intValue() );
            }

            integer = mExif.getTagIntValue( ExifInterface.TAG_EXPOSURE_PROGRAM );
            if( null != integer ) {
                exposureProgram = processExposureProgram( integer.intValue() );
            }

            integer = mExif.getTagIntValue( ExifInterface.TAG_EXPOSURE_MODE );
            if( null != integer ) {
                exposureMode = processExposureMode( integer.intValue() );
            }

            integer = mExif.getTagIntValue( ExifInterface.TAG_SENSING_METHOD );
            if( null != integer ) {
                sensingMethod = processSensingMethod( integer.intValue() );
            }

            integer = mExif.getTagIntValue( ExifInterface.TAG_SCENE_CAPTURE_TYPE );
            if( null != integer ) {
                sceneCaptureType = processSceneCaptureType( integer.intValue() );
            }
            ...
            double[] latlong = mExif.getLatLongAsDoubles();
            ...
            if( null != latlong ) {
                if( latlong[0] != INVALID_LATLNG ) {
                    ...
                    Log.d( LOG_TAG, "lon: " + longitude );
                    Log.d( LOG_TAG, "latitude: " + latitudeString );
                    Log.d( LOG_TAG, "longitude: " + longitudeString );
                }
